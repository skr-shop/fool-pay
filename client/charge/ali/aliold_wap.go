...
type AliOldWapCharge struct {
	*ChargeClient
}

func NewAliOldWapCharge(configData common.BaseConfig) *AliOldWapCharge {
	temp := &AliOldWapCharge{}
...
func (wpc *AliOldWapCharge) BuildData() string {
...
// ToURL
func (wpc *AliOldWapCharge) ToURL(m map[string]string) string {
	var buf []string
	for k, v := range m {
		if v == "" {
			continue
...
func (wpc *AliOldWapCharge) BuildResData() interface{} {
	return wpc.AliResResult
}

func (wpc *AliOldWapCharge) GetSignType() string {
...
func (wpc *AliOldWapCharge) IsOldPay() bool {
	return true
}
